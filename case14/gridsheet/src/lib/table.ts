...
export class Table implements UserTable {
  ...
  private _update({
    ...
    //console.log("table update", mainid)
...
*/
    ...
    if (this.wire.onEdit && Object.keys(diff).length > 0) {
      ...
      //this.wire.onEdit({ table: this.__raw__.trim(updatedArea) , mainid: mainid});  // GUSA
      this.wire.onEdit({ table: this.__raw__.trim(updatedArea) , id: mainid});  // GUSA
    }
    //this.clearSolvedCaches();
    return {
      diffBefore,
  ...
  public write(props: {
    ...
    const { point, value , mainid } = props;
    ...
    //console.log("table.write", mainid)
    return this.update({
      ...props,
      diff,
      partial: true,
