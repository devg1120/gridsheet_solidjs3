...
export function GridSheetPassive({
  ...
  hub: initialHub,
  //panel_split=false,
  panel_split = ()=> { return false;},
  //split_ratio=[100, 100, 100],
  split_ratio = ()=> { return [50,50,50];},
}: PassiveProps) {
...
*/
  ...
  //let split_ratio = [100,[100,0],0,[100,0] ]
  
  //let r = [100,[100,0],0,[100,0] ]
  //let r = [70,[100,0],30,[100,0] ]
  //let r = [70,[70,30],30,[100,0] ]
  //let r = [70,[70,30],30,[40,60] ]
...
if (panel_split()) {
  ...
  onMount(() => {
    ...
    console.log("mainRef", mainRef.offsetWidth)
    console.log("mainRef", mainRef.offsetHeight)
    set_c11_width( mainRef.offsetWidth*(r[1][0]/100) )
    set_c12_width( mainRef.offsetWidth*(r[1][1]/100) )
    set_c21_width( mainRef.offsetWidth*(r[3][0]/100) )
    set_c22_width( mainRef.offsetWidth*(r[3][1]/100) )

    set_c11_height( mainRef.offsetHeight*(r[0]/100) )
...
createEffect(() => {
      if (panel_split()) {
        ...
        // not panel split
          ...
          set_c11_height( mainRef.offsetHeight -2)
          //set_c11_height( mainRef.height -2)
          //set_c11_height( 100)
          //set_c12_height( mainRef.offsetHeight )
          //set_c21_height( 0 )
          //set_c22_height( 0 )
         ...
         //console.log(" split reset .....")
           const el = mainRef.querySelector("#" + table_name + "-A" + "_Tabular");
           //el.scrollTo(1,1)
           //el.scrollTo(0,0)
           el.scrollTop = 1
           el.scrollTop = 0
...
const h_resize = (id, size) => {
  ...
  const syncScroll = (gsid, ele, top, left) => {
      ...
      if ( gsid == table_name + "-A" ) {
          //console.log("A")
              const elB = mainRef.querySelector("#" + table_name + "-B" + "_Tabular");
              //el.scrollTo(left, top)
              elB.scrollTop = top
              //eleB.scrollTop = top
              const elC = mainRef.querySelector("#" + table_name + "-C" + "_Tabular");
              elC.scrollLeft =left
...
{/*
              <PanelGroup  direction="column">
                   <Panel id="1" initialSize={50} onResize={(size) => v_resize("1", size)}>
                      <PanelGroup>
                        <Panel id="11" initialSize={50} onResize={(size) => h_resize("11", size)}>
                            <Tabular  sheetWidth={c11_width} sheetHeight={c11_height} gsid={gsid+"-A"}  />
			</Panel>
                        <ResizeHandle />
                        <Panel id="12" initialSize={50} onResize={(size) => h_resize("12", size)}>
                            <Tabular sheetWidth={c12_width} sheetHeight={c12_height} gsid={gsid+"-B"}  />
			</Panel>
                      </PanelGroup>
                   </Panel>
                <ResizeHandle />
                   <Panel id="2" initialSize={50}  onResize={(size) => v_resize("2", size)}>
                      <PanelGroup>
                        <Panel id="21" initialSize={50} onResize={(size) => h_resize("21", size)}>
                            <Tabular  sheetWidth={c21_width} sheetHeight={c21_height} gsid={gsid+"-C"}  />
			</Panel>
                        <ResizeHandle />
                        <Panel id="22" initialSize={50} onResize={(size) => h_resize("22", size)}>
                            <Tabular  sheetWidth={c22_width} sheetHeight={c22_height} gsid={gsid+"-D"}  />
			</Panel>
                      </PanelGroup>
                   </Panel>
